...
//	
    ...
    mThreadExecutor = cutils::ThreadPoolExecutor::Create(
            cutils::ThreadPoolExecutorOptions{
                    4, cutils::ThreadPoolExecutorOptions::ThreadExecutorPerformance::HIGH,
                    "preview_"
            }
    );
    EXIT();
...
int UVCPreview::setPreviewSize(int width, int height, int min_fps, int max_fps, int mode,
    ...
    if ((requestWidth != width) || (requestHeight != height) || (requestMode != mode)) {
        ...
        LOGD("set preview format %s, mfps %d, ret %d", !requestMode ? "YUYV" : "MJPEG", max_fps,
             result);
    }
    ...
    if (LIKELY(!result)) {
        ...
        if (frameMode) {
            ...
            for (; LIKELY(isRunning());) {
                ...
                auto handle_mjpeg= [frame_mjpeg, this](){
                    uvc_frame_t *frame = NULL;
                    uvc_error_t handle_ret;
                    if (LIKELY(frame_mjpeg)) {
                        frame = get_frame(frame_mjpeg->width * frame_mjpeg->height * 2);
                        //TODO:@kai using mjpeg2rgbx instead
                        handle_ret = uvc_mjpeg2yuyv(frame_mjpeg, frame);   // MJPEG => yuyv
                        recycle_frame(frame_mjpeg);
                        if (LIKELY(!handle_ret)) {
                ...
                if(mThreadExecutor) {
                    mThreadExecutor->Schedule(handle_mjpeg);
                }
            }
        } else {
            // yuyv mode
            ...
            for (; LIKELY(isRunning());) {
                ...
                if (LIKELY(frame)) {
                    auto preview_handle = [frame, this](){
                        auto prev_frame = draw_preview_one(frame, &mPreviewWindow, uvc_any2rgbx, 4);
                        addCaptureFrame(prev_frame);
                    };

                    if(mThreadExecutor) {
                        mThreadExecutor->Schedule(preview_handle);
                    }
                }
