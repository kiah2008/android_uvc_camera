...
#pragma message("<Opencv4.2> compile android version with clang!!")
    template<typename T>
...
#else  // linux or other platform
#pragma message("<Opencv4.2> compile linux version with clang!!")
    template<typename T>
    static std::true_type check(typename std::is_same<typename \
            std::decay<decltype(std::declval<T>().operator()(std::declval<Args>()...))>\
            ::type, Ret>::type*);
    ...
    typedef decltype(check<C>(0)) type;
#endif  // platform end
#else  // other compiler, gcc etc.
#pragma message("<Opencv4.2> compile with gcc!!")
    template<typename T>
    static CV_CONSTEXPR std::true_type check(typename std::is_same<typename \
            std::decay<decltype(std::declval<T>().operator()(std::declval<Args>()...))>\
            ::type, Ret>::type*);
